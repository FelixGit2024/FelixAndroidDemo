...
public class TrafficStatsContentProvider extends ContentProvider {
    private final String TAG = "TrafficStatsContentProvider";

    private TrafficStatsDbHelper dbHelper;
    private static final UriMatcher uriMatcher = new UriMatcher(UriMatcher.NO_MATCH);
    
    static {
        String AUTHORITY= "com.firstproject.androiddemofx.provider.trafficstats.TrafficStatsContentProvider";
        //当URI的authority和路径匹配时，返回代码1，通常用于查询整个表
        uriMatcher.addURI(AUTHORITY, TrafficStatsContract.TABLE_NAME, 1);
        //当URI的authority和路径匹配时，返回代码2，通常用于查询表中的特定记录，这里的#是通配符，匹配任意数字，通常用于指定单个记录的ID
        uriMatcher.addURI(AUTHORITY, TrafficStatsContract.TABLE_NAME + "/#", 2);
    }
    /**
    ...
    public boolean onCreate() {
        dbHelper = new TrafficStatsDbHelper(getContext());
        return true;
    }
    ...
    public Cursor query(@NonNull Uri uri, @Nullable String[] projection, @Nullable String selection, @Nullable String[] selectionArgs, @Nullable String sortOrder) {
        Log.d(TAG, "Query function [ uri:" + uri + ",projection:" + Arrays.toString(projection) + ",selection:" + selection + ",selectionArgs:" + Arrays.toString(selectionArgs) + ",sortOrder:" + sortOrder + " ]");
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Cursor cursor = db.query(
                TrafficStatsContract.TABLE_NAME,
                projection,
                selection,
                selectionArgs,
                null,
    ...
    public Uri insert(@NonNull Uri uri, @Nullable ContentValues values) {
        ...
        long id = db.insert(TrafficStatsContract.TABLE_NAME, null, values);
    ...
    public int delete(@NonNull Uri uri, @Nullable String selection, @Nullable String[] selectionArgs) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        int count = db.delete(TrafficStatsContract.TABLE_NAME, selection, selectionArgs);
        getContext().getContentResolver().notifyChange(uri, null);
        return count;
    }
    ...
    public int update(@NonNull Uri uri, @Nullable ContentValues values, @Nullable String selection, @Nullable String[] selectionArgs) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        int rowsUpdated = db.update(TrafficStatsContract.TABLE_NAME, values, selection, selectionArgs);
        if (rowsUpdated > 0) {
            getContext().getContentResolver().notifyChange(uri, null);
        }
        return rowsUpdated;
